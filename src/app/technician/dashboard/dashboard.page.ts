...
export class DashboardPage implements OnInit, OnDestroy {
  ...
  private async loadUserAndProfile() {
    try {
      const user = await this.authService.refreshUser().toPromise();
      if (user) {
        this.user = user;
  ...
  private async restoreWorkerStatus() {
    ...
    if (this.isWorkerOnline) {
      ...
      if (activeTask) this.locationService.startTracking(activeTask.id);
    }
  }
  ...
  getTotalTasks(): number {
    return this.tasks.filter((t) =>
      ['assigned', 'accepted', 'on-going', 'completed'].includes(t.status),
    ).length;
  }
  ...
  openLocationMap(task: TaskList, event: Event) {
  ...
  async initLocationMap() {
    ...
    const mapEl = document.getElementById('dashboard-location-map');
    if (!mapEl) return;
  ...
  goToTaskList() {
    this.navCtrl.navigateForward(['/technician/tabs/tasks']);
  }
